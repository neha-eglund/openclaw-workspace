...
def main():
    ...
    r.check("5.  Main message includes key deliveries",
            skill_contains(s, r"KEY DELIVERIES.*Main message|Main message.*KEY DELIVERIES", I | D))

    r.check("6.  Main message: only 🔴 blockers under Needs Attention",
            skill_contains(s, r"NEEDS ATTENTION|only 🔴|🔴.*only", I))
    ...
    r.check("8.  Thread 2 = stale items + action items + contributors",
            skill_contains(s, r"Thread reply 2.*[Ss]tale.*action|Thread reply 2.*action.*contributor", I | D))
    ...
    r.check("11. Board flow: Done this week section",
            skill_contains(s, r"Done this week", I))

    r.check("12. Board flow: P0 in flight",
            skill_contains(s, r"P0 in flight", I))
    ...
    r.check("14. Board flow: cycle time in plain English",
            skill_contains(s, r"plain English.*cycle time|cycle time.*plain English|Most PRs shipped", I | D))

    r.check("15. Board flow: time to first review in plain English",
            skill_contains(s, r"first review.*plain English|plain English.*first review|first review.*24h", I | D))

    r.check("16. Board flow: WIP per person",
            skill_contains(s, r"WIP per person|items.*in flight.*per person", I))
    ...
    r.check("20. PR tracking: per-person table (tracked/untracked/total)",
            skill_contains(s, r"Tracked.*Untracked.*Total|per.person table.*tracked", I | D))
